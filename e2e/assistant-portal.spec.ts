...
test.describe('Assistant Portal Flow', () => {
    ...
    const mockEmail = 'test+assistant@example.com';

    test.beforeEach(async ({ page }) => {
        // Mock Supabase RPC calls
        await page.route('**/rest/v1/rpc/accept_assistant_invite', async (route) => {
            await route.fulfill({
                status: 200,
    ...
    test('Guest user should see signup form when visiting invite link', async ({ page }) => {
