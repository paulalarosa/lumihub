...
export const generateClientPDF = (client: Client, projects: ReportProject[]) => {
    ...
    doc.text(`Gerado em: ${format(new Date(), 'dd/MM/yyyy', { locale: ptBR })}`, 130, 32);
    ...
    if (!projects || projects.length === 0) {
        ...
        projects.forEach((project, index) => {
            ...
            doc.setTextColor(colors.textLight);
            doc.setFont('helvetica', 'normal');
            const eventDate = project.event_date ? format(new Date(project.event_date), 'dd/MM/yyyy', { locale: ptBR }) : 'A definir';
            ...
            doc.text(`Total: ${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalValue)}`, 150, yPos + 8);
            ...
            if (services.length > 0) {
                const tableData = services.map((s) => [
                    ...
                    s.quantity || 1,
                    new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(s.unit_price || 0),
                    new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(s.total_price || 0)
                ]);
