...
export function useClientStats() {
    ...
    return useQuery({
        ...
        queryFn: async (): Promise<ClientStats> => {
            if (!organizationId) {
                return { total: 0, active: 0, inactive: 0, newLast30Days: 0, growthRate: 0 };
            }
            ...
            const newLast30Days = clients?.filter(
                (c) => new Date(c.created_at) >= thirtyDaysAgo
            ).length || 0;

            const sixtyDaysAgo = subDays(new Date(), 60);
            const previousPeriod = clients?.filter(
                (c) => new Date(c.created_at) >= sixtyDaysAgo && new Date(c.created_at) < thirtyDaysAgo
            ).length || 0;

            const growthRate = previousPeriod > 0
                ? Math.round(((newLast30Days - previousPeriod) / previousPeriod) * 100)
    ...
    return useQuery({
        ...
        queryFn: async (): Promise<CompanyBreakdown[]> => {
            ...
            data?.forEach((client: Record<string, unknown>) => {
                const company = (client.company as string) || (client.notes as string)?.substring(0, 20) || "Outros";
