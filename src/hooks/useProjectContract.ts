...
export function useProjectContract({ projectId, isClientView, editor }: UseProjectContractProps) {
    ...
    const handleGenerateSignatureLink = async () => {
        ...
        try {
            ...
            toast({ title: 'Sucesso!', description: 'Link de assinatura copiado para a área de transferência.' });
        } catch (_) {
    ...
    const buildSignatureBlock = (signatureDataUrl: string, signedAt: string, contractHash: string) => `
  ...
  `;
    ...
    const handleConfirmSignature = async (signatureDataUrl: string) => {
        ...
        try {
            ...
            const contractHash = uuidv4().replace(/-/g, '').toUpperCase().substring(0, 16);
            const signatureBlock = buildSignatureBlock(signatureDataUrl, signedAt, contractHash);

            if (typeof window !== 'undefined' && window.gtag) {
                window.gtag('event', 'contract_signed', { event_category: 'engagement', event_label: projectId });
            }
            ...
            const pdfBlob = await new Promise<Blob>((resolve, reject) => {
                html2pdf().set(opt as Record<string, unknown>).from(fullHTML).toPdf().output('blob')
                    .then((blob: Blob) => resolve(blob))
                    .catch((err: Error) => reject(err));
            });

            const fileName = `contracts/${projectId}/${Date.now()}_signed.pdf`;
            const { error: uploadError } = await supabase.storage.from('briefing-files').upload(fileName, pdfBlob, { cacheControl: '3600', upsert: false });
            ...
            toast({ title: 'CONTRATO SELADO', description: 'Sua cópia assinada já está disponível.', className: 'bg-black text-white border-neutral-800 font-mono' });
        } catch (error) {
            toast({ title: 'Erro', description: 'Não foi possível finalizar o contrato. Tente novamente.', variant: 'destructive' });
        } finally {
    ...
    const handleDownloadPDF = async () => {
        if (finalPdfUrl) { window.open(finalPdfUrl, '_blank'); return; }
        if (!editor) return;
