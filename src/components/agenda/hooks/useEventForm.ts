...
export function useEventForm({ event, assistants, selectedDate, onSuccess }: UseEventFormProps) {
    const { user } = useAuth();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (event) {
                ...
                await supabase.from('event_assistants').delete().eq('event_id', event.id!);
            } else {
            ...
            try {
                ...
                if (isNoivas) {
                    eventDataForSync.start_time = arrivalTime || makingOfTime || ceremonyTime || '09:00';
                } else {
                ...
                if (syncError) {
                    toast({ title: 'Atenção', description: 'Evento salvo, mas houve erro ao sincronizar com Google.', variant: 'destructive' });
                } else {
