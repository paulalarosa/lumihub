...
export type { Event as SidebarEvent, Assistant as SidebarAssistant, Client as SidebarClient };
...
export function useEventDetailsSidebar({
    open, initialEvent, onEdit, onDelete, onOpenChange, userRole
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const { error } = await supabase.from('events').update(updates).eq('id', initialEvent.id);
            if (error) throw error;

            await supabase.from('event_assistants').delete().eq('event_id', initialEvent.id);
            if (selectedAssistants.length > 0) {
                const assistantsToInsert = selectedAssistants.map(aid => ({ event_id: initialEvent.id, assistant_id: aid }));
                const { error: assistError } = await supabase.from('event_assistants').insert(assistantsToInsert);
    ...
    return {
        ...
        notes, setNotes,
        selectedAssistants,
        openAssistantCombobox, setOpenAssistantCombobox,
        clients,
        availableAssistants,
        fetchingDetail,
        saving,
