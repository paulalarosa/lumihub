...
export const EventDetailsModal = ({ event, isOpen, onClose }: EventDetailsModalProps) => {
    ...
    const handleViewDetails = () => {
        if (event.resource.projectId) {
            ...
            window.open(`https://calendar.google.com/calendar/r/eventedit/${event.resource.googleEventId}`, '_blank');
        }
    };
            ...
            <DialogContent className="bg-neutral-900 border-neutral-800 sm:max-w-md">
                ...
                <div className="space-y-6 mt-4">
                    ...
                    <div className="flex items-start gap-4 text-neutral-300">
                        ...
                        <div>
                            <p className="text-sm text-neutral-500 font-medium uppercase tracking-wider">Data</p>
                    ...
                    {event.resource.location && (
                        <div className="flex items-start gap-4 text-neutral-300">
                            ...
                            <div>
                                <p className="text-sm text-neutral-500 font-medium uppercase tracking-wider">Local</p>
                    ...
                    {event.resource.description && (
                        <div className="flex items-start gap-4 text-neutral-300">
                            ...
                            <div>
                                <p className="text-sm text-neutral-500 font-medium uppercase tracking-wider">Detalhes</p>
                                <p className="text-white text-sm whitespace-pre-wrap">{event.resource.description}</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex items-center gap-3 pt-2">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest ${isGoogleEvent ? 'bg-blue-900/30 text-blue-400 border border-blue-800' :
                            event.resource.serviceType === 'wedding' ? 'bg-[#FFD700]/10 text-[#FFD700] border border-[#FFD700]/30' :
                                event.resource.serviceType === 'social' ? 'bg-[#FF69B4]/10 text-[#FF69B4] border border-[#FF69B4]/30' :
                                    'bg-gray-800 text-gray-300 border border-gray-700'
                            }`}>
                            {isGoogleEvent ? 'Google Calendar' :
                                event.resource.serviceType === 'wedding' ? 'Noiva' :
                                    event.resource.serviceType === 'social' ? 'Social' : 'Teste'}
                        </span>
                    ...
                    {!isGoogleEvent && event.resource.clientPhone && (
                        <div className="pt-4 border-t border-neutral-800">
                            <p className="text-xs text-neutral-500 font-medium uppercase tracking-wider mb-2">Comunicação Rápida</p>
                            <WhatsAppButtons
