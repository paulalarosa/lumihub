...
export function ProjectKanban({ projectId }: ProjectKanbanProps) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (activeTask && STATUSES.includes(newStatus as TaskStatus) && activeTask.status !== newStatus) {
      const originalStatus = activeTask.status;

      // 1. Optimistic Update (Immediate Feedback)
      setTasks((prev) =>
        prev.map((t) =>
          t.id === activeTask.id ? { ...t, status: newStatus as TaskStatus } : t
        )
      );

      // 2. Persist via ProjectService
      const success = await ProjectService.updateTaskStatus(activeTask.id, newStatus);
  ...
  const handleTaskSaved = (savedTask: Task) => {
    ...
    if (exists) {
      setTasks(prev => prev.map(t => t.id === savedTask.id ? kanbanTask : t));
    } else {
