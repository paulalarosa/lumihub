...
export const GeneralAISettings = () => {
  ...
  const handleLocalAIToggle = (checked: boolean) => {
    ...
    toast.success(
      checked
        ? 'System_Protocol: Local_Inference_Online'
        : 'System_Protocol: Cloud_Inference_Online',
    )
  }
  ...
  const currentModels =
    settings.provider === 'gemini' ? geminiModels : openaiModels
  ...
  return (
    <div className="space-y-8 font-mono">
      ...
      <div className="space-y-3">
        ...
        >
          ...
          <SelectContent className="bg-zinc-950 border-white/10 rounded-none">
            <SelectItem
              value="gemini"
              className="text-zinc-400 uppercase text-[10px]"
            >
              Google Gemini Cloud
            </SelectItem>
            <SelectItem
      ...
      <div className="space-y-5 border-t border-white/5 pt-8">
        ...
        />
        <p className="text-[8px] text-zinc-600 uppercase tracking-widest leading-relaxed">
          Maximum response payload size. High buffer allocation allows for deep
          reasoning and extensive document synthesis.
        </p>
      </div>
      ...
      <div className="flex items-center justify-between p-5 bg-zinc-950 border border-white/10 rounded-none mt-4 group hover:border-white/30 transition-all duration-300">
        <div className="flex-1 space-y-1">
          <Label className="text-[11px] font-black uppercase tracking-[0.25em] text-white">
            Total_Privacy_Engine
          </Label>
