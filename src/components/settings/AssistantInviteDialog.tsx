...
export function AssistantInviteDialog() {
    ...
    const { invites, loading, fetchInvites, sendInvite, revokeInvite } = useAssistant();
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
                <Button variant="outline" className="border-[#00e5ff]/20 text-[#00e5ff] hover:bg-[#00e5ff]/10">
            ...
            <DialogContent className="sm:max-w-[500px] bg-[#121212]/95 backdrop-blur-xl border border-white/10 text-white shadow-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-gray-400">
                        Convide assistentes para gerenciar sua agenda e clientes. Eles terão acesso limitado ao seu studio.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="mt-6">
                    <h4 className="text-sm font-medium mb-3 text-gray-400">Convites Enviados</h4>
                    <ScrollArea className="h-[200px] pr-4">
                        <div className="space-y-3">
                            {invites.length === 0 ? (
                                <p className="text-sm text-gray-600 text-center py-4">Nenhum convite enviado.</p>
                            ) : (
                                invites.map((invite) => (
                                    <div key={invite.id} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/5">
