...
import { TemplateType } from '@/services/messageTemplateService'
...
export default function MessageTemplatesSettings() {
  ...
  const loadTemplates = useCallback(async () => {
    ...
    try {
      ...
      logger.error(error, 'MessageTemplatesSettings.loadTemplates', {
        showToast: false,
      })
  ...
  const handleSaveTemplate = async (type: TemplateType, content: string) => {
    ...
    try {
      ...
      logger.error(error, 'MessageTemplatesSettings.handleSaveTemplate', {
        showToast: false,
      })
  ...
  if (loading)
    return (
      <div className="p-8 text-center text-muted-foreground font-mono uppercase">
        Carregando modelos...
      </div>
    )
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-muted/10 border border-border p-4 mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <span
                    key={v}
                    className="px-2 py-1 bg-background border border-border text-[10px] font-mono text-foreground"
                  >
                    {v}
                  </span>
                ))}
            ...
            {(Object.keys(LABELS) as TemplateType[]).map((type) => (
              <div
                key={type}
                className="space-y-3 p-6 border border-border bg-background/50"
              >
          ...
          <div className="bg-zinc-900/50 p-6 rounded-lg border border-zinc-800 h-fit">
            ...
            <div className="space-y-4">
              <p className="text-sm text-zinc-400">
                ...
                sistema abrirá o seu WhatsApp App ou Web pronto para enviar.
              </p>
              ...
              <div className="p-4 bg-zinc-800/50 rounded border border-zinc-700">
                ...
                <Button
                  onClick={() => {
                    if (!userPhone) {
                      toast({
                        title: 'Seu perfil não tem telefone cadastrado',
                        variant: 'destructive',
                      })
                      return
                    }
                    const text = encodeURIComponent(
                      'Olá! O sistema do KONTROL está conectado com sucesso! 🚀',
                    )
                    window.open(
              ...
              <div className="text-xs text-zinc-500 mt-4 border-t border-zinc-800 pt-4">
                * Para envios 100% automáticos sem abrir o app, será necessária
                a integração via API (Fase 2).
              </div>
            </div>
          </div>
        </div>
