...
import {
  ExternalLink, Bell, RefreshCw, Loader2,
  ArrowDownToLine, ArrowUpFromLine
} from 'lucide-react';
import { format } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { useIntegrations, type NotificationSettings } from '@/hooks/useIntegrations';
  ...
  return (
    <div className="space-y-6">
      <Card className="border-border bg-card rounded-none shadow-none">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 font-serif text-lg">Sincronização</CardTitle>
          <CardDescription className="font-mono text-[10px] uppercase tracking-widest">Calendário e Agenda Digital</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="p-6 border border-border bg-background/50 space-y-4">
            <div className="flex items-center justify-between">
                ...
                <div>
                  ...
                  {googleIntegration ? (
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="rounded-none border-green-500/50 text-green-500 bg-green-500/10 font-mono text-[10px] uppercase">CONECTADO</Badge>
              ...
              {googleIntegration ? (
                <Button variant="outline" size="sm" onClick={() => ig.disconnectCalendar(googleIntegration.id, 'google')}
                  className="rounded-none font-mono text-xs uppercase tracking-widest hover:bg-destructive hover:text-white border-border">
                  Desconectar
                </Button>
              ) : (
                <Button onClick={ig.connectGoogleCalendar} disabled={ig.connecting}
                  ...
                  {ig.connecting ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <ExternalLink className="h-4 w-4 mr-2" />}
                  CONECTAR
                </Button>
              )}
            </div>
            ...
            {googleIntegration && (
              <div className="flex flex-wrap gap-2 pt-4 border-t border-border">
                <Button variant="outline" size="sm" onClick={() => ig.syncCalendar('from-google')} disabled={ig.syncing !== null}
                  className="rounded-none border-border font-mono text-[10px] uppercase tracking-widest">
                  {ig.syncing === 'from-google' ? <Loader2 className="h-3 w-3 mr-2 animate-spin" /> : <ArrowDownToLine className="h-3 w-3 mr-2" />}
                  Importar
                </Button>
                <Button variant="outline" size="sm" onClick={() => ig.syncCalendar('to-google')} disabled={ig.syncing !== null}
                  className="rounded-none border-border font-mono text-[10px] uppercase tracking-widest">
                  {ig.syncing === 'to-google' ? <Loader2 className="h-3 w-3 mr-2 animate-spin" /> : <ArrowUpFromLine className="h-3 w-3 mr-2" />}
                  Exportar
                </Button>
              </div>
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-center justify-between p-4 border border-border bg-muted/5 opacity-50">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-blue-500/10 flex items-center justify-center border border-blue-500/20"><span className="font-bold text-blue-500">O</span></div>
                <div><p className="font-serif">Outlook</p><p className="text-[10px] font-mono uppercase text-muted-foreground">Em Breve</p></div>
              </div>
            </div>
            <div className="flex items-center justify-between p-4 border border-border bg-muted/5 opacity-50">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-white/10 flex items-center justify-center border border-white/20"><span className="font-bold">A</span></div>
                <div><p className="font-serif">Apple</p><p className="text-[10px] font-mono uppercase text-muted-foreground">Em Breve</p></div>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Card className="border-border bg-card rounded-none shadow-none">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 font-serif text-lg">Notificações</CardTitle>
          <CardDescription className="font-mono text-[10px] uppercase tracking-widest">Alertas por E-mail</CardDescription>
        </CardHeader>
        <CardContent className="space-y-8">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <Label className="font-mono text-xs uppercase">Ativar notificações por e-mail</Label>
              <p className="text-xs text-muted-foreground">Receba resumos diários e alertas importantes</p>
            </div>
            <Switch checked={ig.notificationSettings.email_enabled}
              onCheckedChange={checked => ig.setNotificationSettings({ ...ig.notificationSettings, email_enabled: checked })} />
          </div>
              ...
              <div className="border-t border-border pt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {[1, 2, 3, 5, 7, 14, 30].map(day => (
                    <button key={day} type="button" onClick={() => ig.toggleReminderDay(day)}
                      className={`px-4 py-2 text-xs font-mono border transition-colors ${ig.notificationSettings.reminder_days.includes(day)
              ...
              <div className="border-t border-border pt-6 space-y-4">
                ...
                {[
                  ...
                  <div key={item.key} className="flex items-center justify-between py-2">
                    ...
                    <Switch checked={ig.notificationSettings[item.key as keyof NotificationSettings] as boolean}
                      onCheckedChange={checked => ig.setNotificationSettings({ ...ig.notificationSettings, [item.key]: checked })} />
                  </div>
                ))}
              </div>
            </>
          ...
          <div className="flex justify-end pt-4 border-t border-border">
            <Button onClick={ig.saveNotificationSettings} disabled={ig.saving}
