...
export async function decryptContractData(encryptedData: string): Promise<string> {
  try {
...
export class ContractPrivacyManager {
  ...
  validateEncryptedId(id: string): boolean {
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}-[0-9a-f]{8}$/i;
