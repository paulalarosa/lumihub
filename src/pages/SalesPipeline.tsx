...
export const SalesPipeline = () => {
    ...
    const leadsByStage = stages?.reduce((acc: any, stage: any) => {
        acc[stage.id] = leads?.filter((lead: any) => lead.current_stage_id === stage.id) || [];
        return acc;
    }, {}) || {};
    ...
    return (
        <div className="container mx-auto p-4 md:p-8 h-[calc(100vh-4rem)] flex flex-col">
                ...
                <div className="flex gap-2">
                    {stages?.length === 0 && (
                        <Button
                            variant="destructive"
                            onClick={async () => {
                                const { error } = await supabase.rpc('create_default_pipeline_stages', { p_user_id: user?.id });
                    ...
                    <Button variant="outline" className="hidden sm:flex bg-transparent border-neutral-700 text-neutral-300 hover:text-white hover:bg-neutral-800">
                        <TrendingUp className="w-4 h-4 mr-2" />
                        Analytics
                    </Button>
                    <Button onClick={() => setIsCreating(true)} className="bg-white text-black hover:bg-neutral-200">
                ...
                <div className="flex-1 overflow-x-auto overflow-y-hidden">
                    <div className="flex gap-4 h-full min-w-max pb-4">
                        {stages?.map((stage: any) => (
                            <div key={stage.id} className="w-80 flex flex-col h-full bg-neutral-900/30 rounded-lg border border-neutral-800/50">
                                {/* Stage Header */}
                                <div
                                ...
                                >
                                    <div className="flex items-center justify-between mb-1">
                                        <h3 className="font-semibold text-white truncate px-1" title={stage.name}>{stage.name}</h3>
                                ...
                                <Droppable droppableId={stage.id}>
                                    {(provided, snapshot) => (
                                        <div
                                            ...
                                            className={`flex-1 p-2 space-y-2 overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-700 scrollbar-track-transparent ${snapshot.isDraggingOver ? 'bg-neutral-800/20' : ''
                                                }`}
                                        >
            ...
            {/* Dialogs */}
            <CreateLeadDialog isOpen={isCreating} onClose={() => setIsCreating(false)} />
        </div>
    );
};
