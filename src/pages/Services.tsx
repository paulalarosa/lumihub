      ...
      <div className="flex-1 overflow-y-auto">
        <main className="p-6 md:p-10 max-w-7xl mx-auto space-y-12">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/20 pb-6">
            <div>
              <h1 className="text-4xl lg:text-5xl font-serif text-white uppercase tracking-tighter">
                {t('pages.services.title')}
              </h1>
              <p className="text-white/50 mt-2 font-mono text-xs uppercase tracking-widest">
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <Skeleton
                  key={i}
                  className="h-48 w-full bg-white/5 rounded-none"
                />
              ))}
            </div>
                  ...
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity absolute top-4 right-4 bg-black border border-white p-1 z-10">
                      <Button
                        size="icon"
                        variant="ghost"
                        className="h-6 w-6 text-white hover:bg-white hover:text-black rounded-none"
