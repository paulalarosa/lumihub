...
export default function Dashboard() {
  ...
  if (!d.user) return <div className="p-8 text-center text-white">Carregando sessão...</div>;
  if (d.orgLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#050505]">
        <motion.div animate={{ rotate: 360 }} transition={{ duration: 2, repeat: Infinity, ease: 'linear' }}
        ...
        <div className="col-span-1 md:col-span-4 lg:col-span-6 lumi-card p-6 border border-white/20 rounded-none mt-6">
          ...
          <div className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={d.originStats} cx="50%" cy="50%" innerRadius={60} outerRadius={80} fill="#8884d8" paddingAngle={5} dataKey="value" stroke="none">
                  {d.originStats.map((_entry, index) => (
                    <Cell key={`cell-${index}`} fill={['#FFFFFF', '#333333', '#666666', '#999999', '#CCCCCC'][index % 5]} />
                  ))}
                </Pie>
                <Tooltip contentStyle={{ backgroundColor: '#000', border: '1px solid #333' }} itemStyle={{ color: '#fff', fontSize: '12px', textTransform: 'uppercase' }} />
                <Legend verticalAlign="middle" align="right" layout="vertical" iconType="circle" wrapperStyle={{ fontSize: '12px', textTransform: 'uppercase', color: '#fff' }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        </div>
