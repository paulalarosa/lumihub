...
export default function MFAEnrollment() {
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            if (challengeData) {
                const verifyData = await verify(enrollmentData.id, challengeData.id, verificationCode);
                if (verifyData) {
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-4">
                    ...
                    <p className="text-sm text-gray-400">
                        Salve estes códigos em um lugar seguro. Você pode usá-los para recuperar o acesso à sua conta caso perca seu dispositivo autenticador.
                        <br />
                        <span className="text-xs text-red-400 mt-1 block">Atenção: Estes códigos são mostrados apenas uma vez. (Simulação: Backend implementation pending for functionality)</span>
                    </p>
                    <div className="grid grid-cols-2 gap-2">
                        {codes.map(c => (
                            <div key={c} className="bg-gray-800 p-2 text-center rounded text-mono text-sm tracking-widest">{c}</div>
                        ))}
                    </div>
    ...
    return (
        <div className="space-y-6 max-w-xl">
            <div>
                <h2 className="text-lg font-medium text-white mb-1">Multi-Factor Authentication (MFA)</h2>
                <p className="text-sm text-gray-400">
                    ...
                    <div className="border border-gray-800 rounded-lg p-4">
                        {factors.map(factor => (
                            <div key={factor.id} className="flex items-center justify-between">
                                <div>
                                    <p className="font-medium text-white">Aplicativo Autenticador (TOTP)</p>
                                    <p className="text-xs text-gray-500">Adicionado em {new Date(factor.created_at).toLocaleDateString()}</p>
                                    <p className="text-xs text-gray-600 mt-1">Status: {factor.status}</p>
                                </div>
            ...
            {enrollmentData && (
                <div className="space-y-6 border border-gray-800 rounded-lg p-6 bg-gray-900/50">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-white font-medium">2. Digite o código de 6 dígitos</h3>
                        ...
                        <Button
                            ...
                            className="w-full"
                        >
                            {isVerifying ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Verificar e Ativar"}
                        </Button>
                    </div>
