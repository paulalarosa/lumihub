...
export const ApiStatusDashboard = () => {
    ...
    const getLevelBadge = (level: string) => {
        switch (level) {
            ...
            case "warn": return <Badge variant="secondary" className="bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500/20">ALERTA</Badge>;
            default: return <Badge variant="outline" className="text-blue-400">INFO</Badge>;
        }
    };
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                {integrations.map((integration) => (
                    <Card key={integration.name} className="bg-black/40 border-white/10 backdrop-blur-md">
                ...
                <CardContent>
                    <ScrollArea className="h-[400px] w-full rounded-md border border-white/10 p-4">
                        <div className="space-y-4">
                            {isLoading ? (
                                <div className="text-center text-muted-foreground py-8">Carregando logs...</div>
                            ) : systemLogs?.length === 0 ? (
                                <div className="text-center text-muted-foreground py-8">Nenhum log registrado recentemente.</div>
                            ) : (
                                systemLogs?.map((log) => (
