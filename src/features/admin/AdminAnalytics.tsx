    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/50 text-sm font-mono uppercase tracking-widest mt-1">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <Skeleton key={i} className="h-28 bg-white/5 rounded-none" />
          ))}
        </div>
      ) : (
        <>
            ...
            {[
              {
                ...
                value: `R$ ${(stats?.monthly_revenue || 0).toLocaleString('pt-BR')}`,
                growth: stats?.growth_revenue_percentage || 0,
                icon: BarChart3,
              },
              {
                label: 'Total Clientes',
                value: stats?.total_clients || 0,
                growth: stats?.growth_client_percentage || 0,
                icon: PieIcon,
              },
              {
              ...
              <Card
                key={i}
                className="bg-black border border-white/10 rounded-none hover:border-white/30 transition-colors"
              >
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {metric.growth !== null && (
                      <span
                        className={`text-xs font-mono flex items-center gap-1 ${isPositive(metric.growth) ? 'text-green-400' : 'text-red-400'}`}
                      >
