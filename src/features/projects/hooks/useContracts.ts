  ...
  const handleArchitectMode = async () => {
    ...
    if (!project?.client) {
      toast({
        variant: 'destructive',
        title: 'Erro',
        description: 'Dados do cliente não carregaram. Recarregue a página.',
      })
      return
    }
    ...
    try {
      ...
      const servicesList =
        projectServices.map((s) => s.service?.name || 'Serviço').join(', ') ||
        'Serviços Gerais'
      ...
      if (data?.text) {
        ...
        toast({
          title: 'Contrato Completo Gerado!',
          description: 'Estrutura criada com sucesso.',
        })
      }
    } catch (error) {
      logger.error(error, 'useContratos.handleArchitectMode', {
        showToast: false,
      })
      toast({
        variant: 'destructive',
        title: 'Erro na IA',
        description: 'Falha ao gerar o contrato.',
      })
    } finally {
      setLoadingAI(false)
    }
  ...
  const handleEditorMode = async () => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: 'Erro na IA',
        description: 'Falha ao editar o contrato.',
      })
    } finally {
