...
export function KhaosAgent() {
    ...
    const PROMPT_STARTERS = [
        { label: "Ver Agenda", icon: <Sparkles className="h-3 w-3" />, prompt: "Como está minha agenda para hoje?" },
        { label: "Criar Lead", icon: <User className="h-3 w-3" />, prompt: "Quero criar um novo lead para um casamento." },
        { label: "Resumo Financeiro", icon: <Shield className="h-3 w-3" />, prompt: "Me dê um resumo do meu faturamento deste mês." },
    ];
        ...
        <div className="flex flex-row h-full bg-black text-white overflow-hidden border-l border-white/5 relative">
            <div className="flex flex-col flex-1 h-full overflow-hidden">
                ...
                <div className="p-4 border-b border-white/5 bg-zinc-950/50 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="space-y-0.5">
                            <h2 className="text-[11px] font-mono tracking-[0.2em] text-white uppercase font-bold">Lumi Protocol</h2>
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[9px] font-mono text-emerald-500/50 uppercase">v3.1.2</span>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <div className="px-2 py-0.5 border border-blue-500/20 bg-blue-500/5 flex items-center gap-1.5 rounded-none">
                            ...
                            <span className="text-[8px] font-mono text-blue-300 uppercase tracking-tighter">Encrypted</span>
                        </div>
                    </div>
                </div>
                ...
                <ChatContainer className="flex-1 min-h-0 bg-transparent relative">
                    <div className="flex flex-col flex-1 p-6 space-y-8 overflow-y-auto custom-scrollbar scroll-smooth" ref={scrollRef}>
                        {messages.length === 0 && (
                            <div className="flex flex-col items-center justify-center h-full space-y-8">
                                <div className="flex flex-col items-center space-y-4 opacity-20 grayscale">
                                    ...
                                    <div className="space-y-1 text-center">
                                        ...
                                        <p className="text-[9px] font-mono uppercase tracking-widest">Protocolo de Assistência Híbrida Ativo</p>
                                    </div>
                                </div>
                                    ...
                                    <div className="flex-1 space-y-4 min-w-0">
                                        ...
                                        {m.reasoning && (
                                            <MessageReasoning
                                                ...
                                                isReasoning={isGenerating && m === messages[messages.length - 1]}
                                            />
                                        )}
                        ...
                        {isGenerating && !messages[messages.length - 1]?.reasoning && (
                            <div className="flex items-center gap-3 opacity-30 px-12">
                                ...
                                <span className="text-[9px] font-mono uppercase tracking-[0.2em]">Neural Processing</span>
                            </div>
                        )}
                    </div>
                </ChatContainer>
                            ...
                            <div className="flex items-center justify-between px-4 py-2 bg-black/40 border-t border-white/5">
                                <div className="flex gap-4">
                                    <div className="flex items-center gap-1.5 opacity-40 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <span className="text-[9px] font-mono uppercase tracking-widest text-emerald-500">Local_READY</span>
                                    </div>
                                    <div className="flex items-center gap-1.5 opacity-20 group-hover:opacity-60 transition-opacity">
                                        <div className="h-1.5 w-1.5 rounded-full bg-blue-500" />
                                        <span className="text-[9px] font-mono uppercase tracking-widest text-blue-500">Cloud_BYOK</span>
                                    </div>
                                </div>
