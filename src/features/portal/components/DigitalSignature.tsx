...
export function DigitalSignature({ isOpen, onClose, contract, onSigned }: DigitalSignatureProps) {
    ...
    const save = async () => {
        ...
        try {
            ...
            doc.text("Contrato de Prestação de Serviços - Cópia Assinada", 105, 30, { align: "center" });
            doc.text(`Assinado em: ${new Date().toLocaleDateString('pt-BR')} às ${new Date().toLocaleTimeString('pt-BR')}`, 105, 35, { align: "center" });
            ...
            const { error: updateError } = await supabase
                ...
                .update({
                    ...
                    signature_data: JSON.stringify({ pdf_url: publicUrl, signed_by: 'client' })
                })
                .eq('id', contract.id);
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-md bg-white text-black">
                <DialogHeader>
                    <DialogTitle className="font-serif uppercase text-center">Confirmar Assinatura</DialogTitle>
                ...
                <DialogFooter className="flex gap-2 sm:justify-center mt-4">
                    ...
                    >
                        {isSaving ? <Loader2 className="w-3 h-3 mr-2 animate-spin" /> : <Check className="w-3 h-3 mr-2" />}
