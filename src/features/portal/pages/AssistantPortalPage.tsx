...
const AssistantPortalPage = () => {
      ...
      <div className="min-h-screen bg-black text-white font-mono flex items-center justify-center p-6">
        <div className="max-w-md w-full border border-white/20 p-12 text-center bg-black">
          ...
          <Button
            ...
            className="rounded-none w-full border-white/20 text-white hover:bg-white hover:text-black font-mono text-xs uppercase tracking-widest h-12"
          >
            TERMINATE_SESSION
          </Button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-mono selection:bg-white selection:text-black pb-24">
        ...
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex flex-col">
            <h1 className="text-xl font-serif text-white tracking-wide">
              KONTROL{' '}
              <span className="text-neutral-600 text-sm font-sans italic">
                / {p.currentAssistantName}
              </span>
            </h1>
          </div>
          <div className="flex items-center gap-4">
            {p.activeAssistants.length > 0 && (
              <div className="hidden md:block w-48">
                <Select
                  value={p.selectedAssistantId}
                  onValueChange={p.setSelectedAssistantId}
                >
                  ...
                  <SelectContent className="bg-black border-neutral-800 rounded-none text-white">
                    ...
                    {p.activeAssistants.map((a) => (
                      <SelectItem
                        ...
                        className="text-[10px] uppercase tracking-wider"
                      >
                        {p.employersMap[a.user_id] || 'Unknown Client'}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
      ...
      <main className="container mx-auto px-4 pt-24">
          ...
          <div className="mb-8 border border-yellow-900/50 bg-yellow-900/10 p-4 flex items-center justify-between animate-pulse">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-yellow-500 uppercase tracking-widest">
                {p.pendingInvites.length}{' '}
                {p.pendingInvites.length === 1
                  ? 'Novo Convite'
                  : 'Novos Convites'}
              </span>
            </div>
            <Button
              ...
              onClick={() => p.setActiveTab('convites')}
            >
              Ver Agora
            </Button>
          </div>
          ...
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <section>
              <div className="flex items-center justify-between mb-4 border-b border-neutral-800 pb-2">
                ...
                {p.nextEvent && (
                  <span className="text-[9px] text-white bg-neutral-900 border border-neutral-800 px-2 py-1 uppercase tracking-widest">
                    CONFIRMED
                  </span>
                )}
              </div>
              {p.nextEvent ? (
                  ...
                  <CardContent className="p-8 relative z-10">
                    <div className="space-y-8">
                      <div className="flex flex-col md:flex-row justify-between md:items-start gap-4 border-b border-white/10 pb-6">
                        <div>
                          ...
                          <h3 className="text-lg font-bold tracking-wide">
                            {p.nextEvent.projects?.name || 'PRIVATE_EVENT'}
                          </h3>
                        ...
                        <div className="text-left md:text-right">
                          ...
                          <p className="text-lg font-serif">
                            {format(
                              parseISO(p.nextEvent.event_date),
                              'dd . MM . yyyy',
                            )}
                          </p>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-8">
                        <div>
                          <p className="text-[9px] text-neutral-500 uppercase tracking-[0.2em] mb-2">
                            CALL_TIME
                          </p>
                          <div className="flex items-center gap-3">
                            <Clock className="w-4 h-4 text-white" />
                            <span className="text-2xl font-light">
                              {p.nextEvent.start_time?.substring(0, 5) || 'TBA'}
                            </span>
                          </div>
                        </div>
                        <div>
                ...
                <div className="border border-neutral-800 bg-neutral-900/20 p-12 text-center rounded-none">
                  ...
                  <p className="text-[9px] text-neutral-700 font-mono uppercase mt-2">
                    STANDBY_MODE_ENGAGED
                  </p>
                </div>
              )}
            </section>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex items-center justify-between border-b border-neutral-800 pb-4">
              ...
              <span className="text-xs font-mono text-neutral-500">
                {p.pendingInvites.length} REQUESTS
              </span>
            </div>
            {p.pendingInvites.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {p.pendingInvites.map((invite) => (
                  <Card
                    key={invite.id}
                    className="bg-neutral-900 border border-neutral-800 rounded-none hover:border-white/30 transition-all"
                  >
                    <CardContent className="p-8 space-y-6">
                      ...
                      <div className="pt-6 border-t border-neutral-800">
                        <Button
                          onClick={() => p.handleAcceptInvite(invite.id)}
        ...
        {p.activeTab === 'agenda' && (
          <AssistantAgenda
            ...
            onMonthChange={p.setCurrentMonth}
          />
        )}
          ...
          <div className="flex flex-col items-center justify-center min-h-[50vh] text-center space-y-6 border border-neutral-800 bg-neutral-900/10 p-12">
            ...
            <div>
              ...
              <p className="text-neutral-500 font-mono text-xs uppercase tracking-widest max-w-xs mx-auto">
                Full financial breakdowns are reserved for Pro Organization
                Accounts.
              </p>
            </div>
            <Button
              onClick={() => {
                p.setSelectedFeature('Financeiro')
                p.setPremiumModalOpen(true)
              }}
              className="rounded-none bg-white text-black font-bold text-xs uppercase tracking-[0.2em] h-12 px-8 hover:bg-neutral-200"
            >
              Unlock Pro Access
            </Button>
          </div>
        ...
        <div className="grid grid-cols-5 h-20">
          {[
            ...
            <button
              ...
              className={`flex flex-col items-center justify-center gap-2 relative group ${p.activeTab === tab ? 'text-white' : 'text-neutral-600'}`}
            >
              <Icon className="h-5 w-5" />
              <span className="text-[8px] font-mono uppercase tracking-[0.2em]">
                {label}
              </span>
              {tab === 'convites' && p.pendingInvites.length > 0 && (
                <span className="absolute top-2 right-4 w-2 h-2 rounded-none bg-yellow-500" />
              )}
              {p.activeTab === tab && (
      ...
      <PremiumFeatureModal
        open={p.premiumModalOpen}
        onOpenChange={p.setPremiumModalOpen}
        featureName={p.selectedFeature}
