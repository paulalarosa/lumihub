...
export default function BrideDashboardPage() {
    ...
    const progress = d.totalContract > 0 ? (d.paidAmount / d.totalContract) * 100 : 0;
        ...
        <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-white selection:text-black flex flex-col">
            <header className="border-b border-neutral-800 bg-[#050505]">
                <div className="container mx-auto px-6 h-20 flex items-center justify-between">
                    <div><span className="font-serif italic text-xl tracking-tight text-white">KONTROL // Client Access</span></div>
                    <Button variant="ghost" className="text-[10px] uppercase tracking-widest text-neutral-500 hover:text-white hover:bg-transparent rounded-none" onClick={d.handleLogout}>
                        <LogOut className="w-3 h-3 mr-2" />{t('bride.logout')}
                    </Button>
                </div>
            </header>
                ...
                <Tabs defaultValue="dashboard" className="w-full">
                    <div className="flex justify-center mb-12">
                        <TabsList className="bg-[#121212] border border-neutral-800">
                            <TabsTrigger value="dashboard" className="data-[state=active]:bg-white data-[state=active]:text-black font-mono text-xs uppercase tracking-widest">Visão Geral</TabsTrigger>
                            <TabsTrigger value="services" className="data-[state=active]:bg-white data-[state=active]:text-black font-mono text-xs uppercase tracking-widest">Serviços</TabsTrigger>
                            <TabsTrigger value="contracts" className="data-[state=active]:bg-white data-[state=active]:text-black font-mono text-xs uppercase tracking-widest">Contratos</TabsTrigger>
                        </TabsList>
                    </div>
                            ...
                            <div className="lg:col-span-5 border-r border-b border-neutral-800 p-8 md:p-12 min-h-[400px] flex flex-col justify-between relative bg-[#050505]">
                                <div className="mb-12">
                                    ...
                                    <h1 className="text-4xl md:text-5xl font-serif text-white leading-tight">
                                        Olá, <span className="italic text-neutral-400">{d.project?.name || d.bride?.name?.split(' ')[0] || 'Noiva'}</span>
                                    </h1>
                                </div>
                                <div>
                                    <p className="text-[10px] uppercase tracking-[0.3em] text-neutral-500 mb-2">{t('bride.countdown')}</p>
                                    <div className="flex items-baseline gap-4">
                                        <span className="text-8xl md:text-9xl font-serif text-white leading-none tracking-tighter">{d.daysLeft}</span>
                                        <span className="text-sm md:text-base font-mono text-neutral-500 rotate-90 origin-left translate-y-[-20px]">{t('bride.days')}</span>
                                    </div>
                                    {d.project?.event_date && (
                                        <div className="mt-8 flex items-center gap-2 text-neutral-400 font-mono text-xs">
                                            <CalendarDays className="w-3 h-3" />
                                            {format(new Date(d.project.event_date), 'dd . MM . yyyy', { locale: ptBR })}
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="lg:col-span-7 grid grid-rows-2">
                                <div className="border-r border-b border-neutral-800 p-8 md:p-12 bg-[#050505]">
                                    ...
                                    <div className="space-y-6 max-h-[250px] overflow-y-auto scrollbar-thin pr-4">
                                            ...
                                            return (
                                                <div key={evt.id} className="flex gap-6 group">
                                                    ...
                                                    <div className="relative flex flex-col items-center">
                                                        ...
                                                        {idx !== d.events.length - 1 && <div className="w-[1px] h-full bg-neutral-900 absolute top-2" />}
                                                    </div>
                                                    <div className="pb-6 pt-0.5">
                                                        <p className={`text-sm uppercase tracking-wider ${isPast ? 'text-neutral-600' : 'text-white'}`}>{evt.title}</p>
                                                        {isPast && (
                                        ...
                                        {d.events.length === 0 && <p className="text-neutral-600 text-xs font-mono">Nenhum evento agendado.</p>}
                                    </div>
                                </div>
                                ...
                                <div className="border-r border-b border-neutral-800 p-8 md:p-12 bg-[#050505] flex flex-col justify-center">
                                    <div className="flex items-center justify-between mb-8">
                                        ...
                                        {remaining <= 0 && d.totalContract > 0 && (
                                            <span className="text-[10px] uppercase tracking-widest border border-[#D4AF37] text-[#D4AF37] px-2 py-1">Quitado</span>
                                        )}
                                    </div>
                                    <div className="grid grid-cols-3 gap-8 mb-6">
                                        ...
                                        <div className="text-center md:text-right">
                                            ...
                                            <p className="text-lg md:text-2xl font-light text-green-500">R$ {d.paidAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                        </div>
                                        <div className="text-center md:text-right">
                                            <p className="text-[10px] uppercase tracking-widest text-neutral-500 mb-1">{t('dashboard.pending')}</p>
                                            <p className="text-lg md:text-2xl font-light text-red-500">R$ {remaining.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                        </div>
                                    </div>
                                    <div className="relative w-full h-1 bg-neutral-900">
                                        <div className="absolute top-0 left-0 h-full bg-white transition-all duration-1000" style={{ width: `${progress}%` }} />
                                    </div>
                                    <div className="mt-2 flex justify-between text-[9px] uppercase tracking-widest text-neutral-600 font-mono">
                                        <span>{t('bride.progress')}</span><span>{Math.round(progress)}%</span>
                                    </div>
                                    {remaining > 0 && (
                                    ...
                                    {d.services.map(svc => (
                                        <div key={svc.id} className="bg-[#0a0a0a] border border-neutral-800 p-6 flex justify-between items-center group hover:border-neutral-700 transition-colors">
                                            ...
                                            <div className="text-right">
                                                <div className={`text-[10px] uppercase tracking-widest border px-2 py-1 ${svc.status === 'paid' ? 'border-green-500 text-green-500' : svc.status === 'partial' ? 'border-yellow-500 text-yellow-500' : 'border-neutral-700 text-neutral-500'}`}>
                                            ...
                                            <div>
                                                {contract.status !== 'cancelled' ? (
                                                    <Button
                                                        ...
                                                        className={contract.status === 'signed' ? 'border-neutral-800 hover:bg-neutral-900 text-neutral-300' : 'bg-white text-black hover:bg-neutral-200'}
                                                        onClick={() => {
