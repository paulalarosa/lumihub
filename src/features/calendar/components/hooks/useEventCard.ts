...
export function useEventCard(event: Event) {
  ...
  const getWhatsAppAction = () => {
    const zonedEventDate = toZonedTime(event.event_date)
    const zonedNow = toZonedTime(new Date())
    const daysUntil = differenceInDays(zonedEventDate, zonedNow)
  ...
  const handleSendWhatsApp = async (recommendedAction: string) => {
    try {
      ...
      const link = generateWhatsAppLink(rawText, {
        ...
        date: formatDate(event.event_date, 'dd/MM'),
