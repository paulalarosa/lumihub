...
export function useClientActions({ fetchClients, clients }: UseClientActionsProps) {
    ...
    const copyPortalLink = async (clientId: string) => {
        try {
            ...
            toast({ title: 'Link do Portal Copiado!', description: 'Envie este link para a noiva.' });
        } catch (e) {
