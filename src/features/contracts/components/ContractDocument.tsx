    ...
    <Document>
        <Page size="A4" style={styles.page}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.contractId}>REF: #{contract.id.slice(0, 8).toUpperCase()}</Text>
            </View>

            {/* Title */}
            <Text style={{ fontSize: 14, fontFamily: 'Helvetica-Bold', textAlign: 'center', marginBottom: 20 }}>
                SERVICE AGREEMENT
            </Text>
            ...
            <View style={{ marginBottom: 20 }}>
                <Text style={styles.text}>
                    This Agreement is entered into on this day of {new Date().toLocaleDateString('pt-BR')}, by and between:
                </Text>
                <Text style={styles.text}>
                    <Text style={{ fontFamily: 'Helvetica-Bold' }}>PROVIDER:</Text> {businessName}
                </Text>
                <Text style={styles.text}>
                    <Text style={{ fontFamily: 'Helvetica-Bold' }}>CLIENT:</Text> {contract.clientName}
                </Text>
                <Text style={styles.text}>
                    <Text style={{ fontFamily: 'Helvetica-Bold' }}>EVENT DATE:</Text> {contract.eventDate}
                </Text>
            </View>
            ...
            {contract.servicesList && contract.servicesList.length > 0 ? (
                <View style={styles.table}>
                    ...
                    <View style={styles.tableRow}>
                        <View style={{ ...styles.tableCol, width: "100%", backgroundColor: '#f0f0f0' }}>
                            <Text style={{ ...styles.tableCell, fontFamily: 'Helvetica-Bold' }}>TOTAL: {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(contract.totalValue)}</Text>
                        </View>
                    </View>
                </View>
            ) : (
                <Text style={styles.text}>
                    Total Agreed Value: {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(contract.totalValue)}
                </Text>
            )}
