...
export default function ContractsPage() {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'signed':
                return <Badge className="bg-white text-black border-white rounded-none font-mono uppercase text-[10px] tracking-widest hover:bg-white/90">ASSINADO</Badge>;
            case 'sent':
                return <Badge className="bg-transparent text-white border-white/40 rounded-none font-mono uppercase text-[10px] tracking-widest hover:border-white">ENVIADO</Badge>;
            default:
                return <Badge className="bg-white/5 text-white/50 border-white/10 rounded-none font-mono uppercase text-[10px] tracking-widest hover:bg-white/10">PENDENTE</Badge>;
        }
    };
    ...
    return (
        <div className="container mx-auto px-4 py-8 space-y-8">
            <div className="flex justify-between items-center border-b border-white/10 pb-6">
                <div>
                    <h1 className="text-3xl font-serif font-light text-white mb-2 tracking-wide uppercase">{t('pages.contracts.title')}</h1>
                    <p className="text-white/40 font-mono text-xs uppercase tracking-widest">{t('pages.contracts.subtitle')}</p>
                </div>
                <Button
            ...
            {loading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {[1, 2, 3].map((i) => (
                        <div key={i} className="h-48 bg-white/5 animate-pulse rounded-none border border-white/10" />
                    ))}
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {contracts.map((contract) => (
                        <Card key={contract.id} className="bg-[#050505] border-white/10 hover:border-white/30 transition-all rounded-none group">
                            ...
                            <CardContent className="pt-4">
                                <div className="space-y-6">
                                    <div className="text-[10px] text-white/30 font-mono uppercase tracking-widest flex items-center gap-2">
                                        ...
                                        CRIADO EM {format(new Date(contract.created_at), "dd MMM yyyy", { locale: ptBR }).toUpperCase()}
                                    </div>
                                    ...
                                    <div className="flex gap-3">
                                        <Button
                                            ...
                                            onClick={() => window.open(`/projects/${contract.project_id}/contract`, '_blank')}
                                        >
                                        ...
                                        {contract.status === 'signed' && (
                                            <Button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    window.open(`/projects/${contract.project_id}/contract`, '_blank');
                                                }}
                                            >
                    ...
                    {contracts.length === 0 && (
                        <div className="col-span-full py-16 text-center border border-dashed border-white/10 rounded-none bg-white/[0.02]">
                            ...
                            <p className="text-white/40 font-mono text-xs uppercase tracking-widest">{t('pages.contracts.no_pending')}</p>
                        </div>
                    )}
                </div>
            )}
