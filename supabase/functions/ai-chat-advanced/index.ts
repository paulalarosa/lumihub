...
serve(async (req) => {
  ...
  try {
    ...
    if (!apiKey) {
      throw new Error(
        'API key não fornecida. Por favor, configure sua chave nas configurações de IA.',
      )
    }
    ...
    const tools = [
      {
        functionDeclarations: [
          ...
          },
          {
            ...
            description:
              'Retorna estatísticas do dashboard (faturamento, eventos, etc)',
            parameters: {
              type: 'object',
              properties: {
                period: {
          ...
          },
          {
            name: 'create_canvas',
            description:
              'Cria um documento/artifact para trabalhar em paralelo (contratos, roteiros, planos)',
            parameters: {
              type: 'object',
              properties: {
                ...
                content: {
                  type: 'string',
                  description: 'Conteúdo inicial em Markdown',
                },
                type: {
                  type: 'string',
    ...
    const chat = aiModel.startChat({
      history: [
        ...
        },
        {
          role: 'model',
          parts: [
            { text: 'Protocolo Khaos iniciado. Como posso otimizar seu dia?' },
          ],
        },
